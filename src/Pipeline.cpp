...
std::unique_ptr<Pipeline> Pipeline::createGaussianBlurPipeline(VulkanContext* context, std::vector<DescriptorSetLayout*> descriptorSetLayouts) {
	std::unique_ptr<Pipeline> pipeline = std::unique_ptr<Pipeline>(new Pipeline());
	pipeline->initGaussianBlur(context, descriptorSetLayouts);
	return pipeline;
}

void Pipeline::initGaussianBlur(VulkanContext* context, std::vector<DescriptorSetLayout*> descriptorSetLayouts) {
	this->context = context;

	auto shaderCode = VulkanUtil::readFile("spv/gaussianBlurVariance.comp.spv");
	VkShaderModule computeShaderModule = createShaderModule(context, shaderCode);
	...
	if (vkCreatePipelineLayout(context->getDevice(), &pipelineLayoutInfo, nullptr, &m_pipelineLayout) != VK_SUCCESS) {
		throw std::runtime_error("failed to create Gaussian blur pipeline layout!");
	}
	...
	if (vkCreateComputePipelines(context->getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_pipeline) != VK_SUCCESS) {
		throw std::runtime_error("failed to create Gaussian blur compute pipeline!");
	}
